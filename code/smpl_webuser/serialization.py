...
def ready_arguments(fname_or_dict):
    ...
    if want_shapemodel:
        ...
        b = dd['betas']
        c = a.dot(b)
        dd['v_shaped'] = c+dd['v_template']
        v_shaped = dd['v_shaped']
        J_tmpx = MatVecMult(dd['J_regressor'], v_shaped[:,0])        
        J_tmpy = MatVecMult(dd['J_regressor'], v_shaped[:,1])        
        J_tmpz = MatVecMult(dd['J_regressor'], v_shaped[:,2])
        stack = ch.vstack((J_tmpx, J_tmpy, J_tmpz))
        dd['J'] = stack.T
        pose_map = posemap(dd['bs_type'])
        pose_map1 = pose_map(dd['pose'])
        pose_map2 = dd['posedirs'].dot(pose_map1)
        dd['v_posed'] = v_shaped + pose_map2
    else:    
