...
def optimize_on_joints(file_path_number,ratio,model,j2d,init_trans,init_rot,prior,n_betas=10,regs=None,conf=None,pyr = None):
    t0=time()
   ...
   # init_pose = np.hstack((init_rot,np.zeros(69)))
    ...
    # set head pose depend on pose[45:48]
    # #
    ...
    # data term: distance between observed and estimated joints in 2D
    ##EJ
    obj_j2d = lambda w, sigma: (w * weights.reshape((-1, 1)) * GMOf((j2d[kinect_ids] - Jtr), sigma))
...
#w2  0.1  1
    ...
    #              'e_3': .0001,
    #              'disp': 0})
    # with open('./fitted_model_%d/just_betas_para_%d.pkl'%(file_path_number,file_path_number),'w') as f:
    #     pickle.dump(sv.betas,f)
    # outmesh_path = './fitted_model_%d/just_betas_%d.obj' % (file_path_number, 1)
    # with open(outmesh_path, 'w') as fp:
    #     for v in sv.r:
    #         fp.write('v %f %f %f\n' % (v[0], v[1], v[2]))
    #     for f in sv.f + 1:  # Faces are 1-based, not 0-based in obj files
    ...
    # tbetas = ch.zeros(10)
    # tbetas[:] = sv.betas.r
    # sv.betas[:] = np.zeros(10)
    # outmesh_path = './fitted_model_%d/just_betas_0_%d.obj' % (file_path_number, 1)
    # with open(outmesh_path, 'w') as fp:
    #     for v in sv.r:
    #         fp.write('v %f %f %f\n' % (v[0], v[1], v[2]))
    #     for f in sv.f + 1:  # Faces are 1-based, not 0-based in obj files
    #         fp.write('f %d %d %d\n' % (f[0], f[1], f[2]))
    with open('./fitted_model_%d/hand_pose_1001.pkl'%(1001),'r') as f:
        just_b = pickle.load(f)
    for ids in range(15):
        ...
        ch.minimize(
            ...
            options={'maxiter': 300,
                     ...
                     'disp': 0})
        tmp_b = sv.betas.r
        sv.betas[:] = just_b['betas'].r
        # para = {'betas':sv.betas,'pose':sv.pose}
        # with open('./fitted_model_%d/kinect_smpl_test_%d.pkl'%(file_path_number,ids),'w') as f:
        #     pickle.dump(para,f)
    ...
    # sv.betas[:] = tbetas.r
    # outmesh_path = './fitted_model_%d/kinect_smpl_test_%d.obj' % (file_path_number, 12)
    # with open(outmesh_path, 'w') as fp:
    #     for v in sv.r:
    #         fp.write('v %f %f %f\n' % (v[0], v[1], v[2]))
    #     for f in sv.f + 1:  # Faces are 1-based, not 0-based in obj files
    ...
    #     if (ids == 0):
    #         objs['betas'] = w2 * betas
    #         objs['pose'] = w1*init_pose
    #     else:
    #         tmpBetas = ch.zeros(10)
    #         tmpBetas[:] = betas.r
    #         objs['betas'] = w2*(betas - tmpBetas)
    #
    #         tmpPoses = ch.zeros(72)
    #         tmpPoses[:] = init_pose.r
    #         objs['pose'] = w2 * (init_pose - tmpPoses)
    #
    #     w2 += 1
    #     objs['j2d'] = obj_j2d(w0, 100)
    #     # w0 += 1
    ...
    # sv.pose[:3] = np.zeros(3)
    # sv.pose[36:39] = np.zeros(3)
    # sv.betas += ch.array(np.hstack((-2, np.zeros(9))))

    # sv.betas[:] = sv.betas.r + np.hstack((np.array([0.5,1]),np.zeros(8)))


    # outmesh_path = './fitted_model_%d/kinect_smpl_%d.obj'%(file_path_number,10)
    # with open(outmesh_path, 'w') as fp:
    #     for v in sv.r:
    #         fp.write('v %f %f %f\n' % (v[0], v[1], v[2]))
    #     for f in sv.f + 1:  # Faces are 1-based, not 0-based in obj files
    ...
    obj = {}
    obj['x'] = (pyr[0] - (thetax*diff_x/ch.absolute(diff_x) - theta_init[0]))
    obj['y'] = (pyr[1] - (thetay*-diff_y/ch.absolute(diff_y) - theta_init[1]))
    obj['z'] = (pyr[2] - (thetaz*diff_z/ch.absolute(diff_z) - theta_init[2]))
    obj['pose'] = pprior(0.1)
    ch.minimize(
        obj,
...
def run_single_fit(file_path_number,
    ...
    # fit
    optimize_on_joints(file_path_number,ratio,model, j2d, init_trans, init_rot, prior, n_betas=n_betas, regs=regs, conf=conf,pyr = pyr)
