...
private:
    ...
    void                   DrawStreams(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight,RGBQUAD* pColorBuffer,UINT16* pDepthBuffer,BYTE* pBodyIndexBuffer,IBody** pBodys,int frameNum,USHORT nMinDepth,USHORT nMaxDepth);

    /// <summary>
    /// Processes new face frames
    /// </summary>
    void                   ProcessFaces(int frameNum, RGBQUAD* pColorBuffer, UINT16* pDepthBuffer, BYTE* pBodyIndexBuffer, IBody** pBodys,USHORT nMinDepth,USHORT nMaxDepth);
	...
	Joint						joints[JointType_Count];//3D joints position
	ColorSpacePoint				joints_position[JointType_Count];//all joints position
